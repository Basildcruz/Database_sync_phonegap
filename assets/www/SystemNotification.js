...
SystemNotification.prototype.newCount = 0; // to keep track of multiple
...
SystemNotification.prototype.createStatusBarNotification = function(
		contentTitle, contentText, tickerText) {
	PhoneGap.exec(null, null, "systemNotification",
			"createStatusBarNotification", [ contentTitle, contentText,
					tickerText ]);
};

SystemNotification.prototype.updateNotification = function(contentText,
		tickerText, number) {
	this.newCount++;
	var contentTitle = "new offer";
	if (this.newCount === 1) {
		this.createStatusBarNotification(contentTitle, contentText, tickerText);
	} else {
		PhoneGap.exec(null, null, "systemNotification", "updateNotification", [
				contentTitle, contentText, this.newCount ]);
...
SystemNotification.prototype.showTickerText = function(tickerText) {
	PhoneGap.exec(null, null, "systemNotification", "showTickerText",
			[ tickerText ]);
};
...
PhoneGap.addConstructor(function() {
	if (typeof (navigator.systemNotification) == "undefined") {
		...
		navigator.systemNotification.touch(); // this ensures that the plugin
												// is added when phonegap kicks
												// off
	}
});